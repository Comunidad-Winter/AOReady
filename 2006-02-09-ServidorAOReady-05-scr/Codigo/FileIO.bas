...
For Hechizo = 1 To NumeroHechizos
    ...
    Hechizos(Hechizo).Area = val(Leer.DarValor("Hechizo" & Hechizo, "Area")) ' GS
    ...
    Hechizos(Hechizo).ExclusivoClase = UCase$(Leer.DarValor("Hechizo" & Hechizo, "ExclusivoClase")) ' [GS] ExclusivoClase
...
For Object = 1 To NumObjDatas
    ...
    If ObjData(Object).ObjType = OBJTYPE_WEAPON Then
            ...
            ObjData(Object).Doble = val(Leer.DarValor("OBJ" & Object, "Doble")) ' GS
...
man:
    MsgBox ("Error durante la carga de mapas.")
    ' [GS] donde fallo ?
    If Fallo = 1 Then MsgBox "Fallo durante la carga del NPC " & MapData(Map, X, Y).NpcIndex & " de bkNPC.dat"
...
AntiAOH = IIf(CStr(GetVar(IniPath & "Ready.ini", "CONFIGURACION", "AntiAOH")) = "1", True, False)
Cliente0113 = IIf(CStr(GetVar(IniPath & "Ready.ini", "CONFIGURACION", "Cliente0113")) = "1", True, False)
NOEdit = IIf(CStr(GetVar(IniPath & "Ready.ini", "CONFIGURACION", "NoEdit")) = "1", True, False)
URL_Soporte = CStr(GetVar(IniPath & "Ready.ini", "CONFIGURACION", "WebSite"))
AutoOroEquip = IIf(CStr(GetVar(IniPath & "Ready.ini", "CONFIGURACION", "AutoOroEquip")) = "1", True, False)
Multiplicador(0) = val(GetVar(IniPath & "Ready.ini", "CONFIGURACION", "ExpMult"))
Multiplicador(1) = val(GetVar(IniPath & "Ready.ini", "CONFIGURACION", "OroMult"))
STAT_MAXELV = val(GetVar(IniPath & "Ready.ini", "USUARIOS", "NivelMaximo"))
If STAT_MAXELV <= 0 Or STAT_MAXELV >= 1001 Then STAT_MAXELV = 199
SeCaenLosItems = IIf(CStr(GetVar(IniPath & "Ready.ini", "USUARIOS", "SeCaenLosItems")) = "1", True, False)
SkillsRapidos = IIf(CStr(GetVar(IniPath & "Ready.ini", "USUARIOS", "SkillsRapidos")) = "1", True, False)
Dados(0) = CByte(GetVar(IniPath & "Ready.ini", "USUARIOS", "MinDados"))
Dados(1) = CByte(GetVar(IniPath & "Ready.ini", "USUARIOS", "MaxDados"))
If Dados(1) < Dados(0) Then Dados(1) = Dados(0)
CfgInvi = CByte(GetVar(IniPath & "Ready.ini", "USUARIOS", "Invisibilidad"))
' [/GS]
...
ServerIp = CStr(GetVar(IniPath & "Server.ini", "INIT", "ServerIp"))
