...
Begin VB.Form frmMain 
   ...
   Caption         =   "Argentum Online - AO Ready"
   ClientHeight    =   5880
   ClientLeft      =   1950
   ClientTop       =   1815
   ClientWidth     =   5190
   ...
   Begin VB.Timer TimerCentinela 
      Interval        =   500
      Left            =   2040
      Top             =   2775
   End
   Begin VB.Timer EventoCentinela 
      Interval        =   60000
      Left            =   1560
      Top             =   2775
   End
   Begin VB.Timer CarcelCenti 
      Interval        =   30000
      Left            =   1080
      Top             =   2775
   End
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.CommandButton Command4 
         Caption         =   "Por Ventanas a Admin's"
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
   ...
   Begin VB.Label CantUsuarios 
      ...
      BackColor       =   &H00C0C0C0&
      BackStyle       =   0  'Transparent
      Caption         =   "Numero de usuarios jugando: 0"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   8.25
         Charset         =   0
...
' GS
Private Sub CarcelCenti_Timer()
Dim UserCarcel As Integer
If Centinela Then
For UserCarcel = 1 To LastUser
    Call NoCenti(UserCarcel)
Next UserCarcel
End If
End Sub

Private Sub EventoCentinela_Timer()

Dim PongoCenti As Integer

On Error GoTo ErrorHandler

Static MinutosSinCentinela As Long

If Not Centinela Then
For PongoCenti = 1 To LastUser
    Call NoPasoCentinela(PongoCenti)
Next PongoCenti
    MinutosSinCentinela = MinutosSinCentinela + 1
    If MinutosSinCentinela >= 15 Then
                Centinela = True
    MinutosSinCentinela = 0
            End If
                
Else
    Centinela = False
    CarcelCenti.Enabled = False
End If

Exit Sub
ErrorHandler:
Call LogError("Error EventoCentinela")
End Sub

Private Sub TimerCentinela_Timer()

On Error GoTo ErrorCenti

Dim Userin As Integer
If Centinela Then
For Userin = 1 To LastUser
    Call LlamoCentinela(Userin)
    Call PasoCentinela(Userin)
Next Userin
CarcelCenti.Enabled = True
End If
...
Exit Sub
ErrorCenti:
Call LogError("Error Centinela " & Err.Number & ": " & Err.Description)
...
Call SendData(ToAdmins, 0, 0, "||Servidor a Admin's> " & BroadMsg.Text & FONTTYPE_SERVER)
...
Me.Caption = "Argentum Online - AO Ready v" & AOReadyVer
Label3.Caption = "AO Ready v" & AOReadyVer
End Sub
...
'         End If
         ...
         If UserList(iUserIndex).flags.Muerto = 0 Then
               ...
               '[Consejeros]
               'If UserList(iUserIndex).flags.Desnudo And UserList(iUserIndex).flags.Privilegios = 0 Then Call EfectoFrio(iUserIndex)    ' [GS] Desactivo el Efecto del Frio
