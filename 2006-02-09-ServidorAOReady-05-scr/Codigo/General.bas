...
' [GS] Impedir volver a ejecutar cuando ya lo esta siendo
If App.PrevInstance = True Then
    MsgBox "El servidor ya esta ejecutado."
    End
End If
' [/GS]
...
' [GS] Version en el Cargando
frmCargando.Label1(0).Caption = "AO Ready v" & AOReadyVer
' [/GS]
...
With frmMain
    ...
    .tLluviaEvent.Enabled = False ' [GS] Desactivo la lluvia
    .FX.Enabled = True
...
'  Else
    ' GS
    If CfgInvi = 1 Then
        Call SendData(ToMapButIndexC, UserIndex, UserList(UserIndex).Pos.Map, "NOVER" & UserList(UserIndex).Char.charindex & ",0")
...
'                Call SendData(ToIndex, i, 0, "||En " & UserList(i).Counters.Salir & " segundos se cerrará el juego..." & FONTTYPE_INFO)
    ...
    If CuentaRegresiva > 0 Then
        ...
        Else
            Call SendData(ToAll, 0, 0, "||YA!!!!!!!!!" & "~255~0~0~1~1")
        End If
...
' GS
Public Sub PasoCentinela(ByVal UserIndex As Integer)
If UserList(UserIndex).flags.UserLogged Then
UserList(UserIndex).flags.PasoCentinela = 1
End If
End Sub

Public Sub NoPasoCentinela(ByVal UserIndex As Integer)
If UserList(UserIndex).flags.UserLogged Then
UserList(UserIndex).flags.PasoCentinela = 0
End If
End Sub

Public Sub NoCenti(ByVal UserIndex As Integer)
If UserList(UserIndex).flags.Centinela = 1 Then
Call CloseSocket(UserIndex)
Call SendData(ToAll, UserIndex, 0, "|| " & UserList(UserIndex).Name & " ha sido expulsado del juego por no haber respondido al llamado del Centinela durante el macreo." & FONTTYPE_INFOBOLD)
Dim nIndex As Integer
nIndex = MapData(UserList(UserIndex).Pos.Map, UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y).NpcIndex
Call QuitarNPC(nIndex)
End If
End Sub

Public Sub LlamoCentinela(ByVal UserIndex As Integer)
If UserList(UserIndex).flags.UserLogged And UserList(UserIndex).flags.PasoCentinela = 0 Then
    If UserList(UserIndex).flags.Trabajando = True Then
        Dim Dondetoy As WorldPos
        Dondetoy.Map = UserList(UserIndex).Pos.Map
        Dondetoy.X = UserList(UserIndex).Pos.X
        Dondetoy.Y = UserList(UserIndex).Pos.Y + 1
        Call CrearNPC(119, UserList(UserIndex).Pos.Map, Dondetoy)
        UserList(UserIndex).flags.Centinela = 1
        UserList(UserIndex).flags.PasoCentinela = 1
        Dim Codigo As Integer
        Codigo = RandomNumber(200, 500)
        UserList(UserIndex).flags.Codigo = Codigo
        Call SendData(ToIndex, UserIndex, 0, "||Un centinela a acudido a ti para comprobar tu macreo, tu codigo de verificacion es: " & Codigo & FONTTYPE_GUILD)
    End If
End If
End Sub
' /GS
