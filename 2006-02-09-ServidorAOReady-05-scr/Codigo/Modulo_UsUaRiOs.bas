...
'        End If
        If MapData(Map, X, Y).UserIndex > 0 And UserIndex <> MapData(Map, X, Y).UserIndex Then
            ...
            If UserList(MapData(Map, X, Y).UserIndex).flags.Invisible = 1 Then
                ' GS
                If CfgInvi = 1 Then
                    Call SendData(ToMapButIndexC, UserIndex, UserList(UserIndex).Pos.Map, "NOVER" & UserList(MapData(Map, X, Y).UserIndex).Char.charindex & ",1")
                    Call SendData(ToIndex, UserIndex, 0, "SIVER" & UserList(MapData(Map, X, Y).UserIndex).Char.charindex & ",1")
                Else
                    Call SendData(ToMap, 0, UserList(UserIndex).Pos.Map, "NOVER" & UserList(MapData(Map, X, Y).UserIndex).Char.charindex & ",1")
                End If
                ' /GS
            End If
        End If
...
Call UpdateUserMap(UserIndex)
        ...
        If (UserList(UserIndex).flags.Invisible = 1 Or UserList(UserIndex).flags.Oculto = 1) And (Not UserList(UserIndex).flags.AdminInvisible = 1) Then
            ' GS
            If CfgInvi = 1 Then
                Call SendData(ToMapButIndexC, UserIndex, UserList(UserIndex).Pos.Map, "NOVER" & UserList(UserIndex).Char.charindex & ",1")
                Call SendData(ToIndex, UserIndex, UserList(UserIndex).Pos.Map, "SIVER" & UserList(UserIndex).Char.charindex & ",1")
            Else
