...
Procesado = True 'ver al final del sub
    ...
    Select Case UCase$(rdata)
        ' [GS] v0.1 comando /CREDITOS
        Case "/CREDITOS"
                Call SendData(ToIndex, UserIndex, 0, "||AO Ready v" & AOReadyVer & tStr & FONTTYPE_WARNING)
                Call SendData(ToIndex, UserIndex, 0, "||Programado por ^[GS]^  || Dateado/Ambientado/Testado por Pada" & tStr & FONTTYPE_SERVER)
                Call SendData(ToIndex, UserIndex, 0, "||Website: www.gs-zone.com.ar" & tStr & FONTTYPE_SERVER)
                Exit Sub
        ' [/GS]
        Case "/ONLINE"
            N = 0
            ...
            End If
            Call SendData(ToIndex, UserIndex, 0, "||" & tStr & "~200~200~200~0~0") ' [GS] v0.2 Me gusta con este color xD
    ...
    ''        Exit Sub
        ...
        Case "/SEGCLAN"
            If UserList(UserIndex).flags.SeguroClan Then
                Call SendData(ToIndex, UserIndex, 0, "||Has desactivado el Seguro de Clan." & FONTTYPE_WARNING)
            Else
                Call SendData(ToIndex, UserIndex, 0, "||Has activado el Seguro de Clan." & FONTTYPE_GUILD)
            End If
...
' GS
    ...
    If UCase$(Left$(rdata, 8)) = "/CHISME " Then
        rdata = Right$(rdata, Len(rdata) - 8)
        If UserList(UserIndex).Stats.GLD < 100000 Then
        Call SendData(ToIndex, UserIndex, 0, "||" & vbWhite & "°" & "No tienes dinero suficiente para que te cuente mis secretos, necesitas al menos 100000 monedas" & "°" & Npclist(UserList(UserIndex).flags.TargetNPC).Char.charindex & FONTTYPE_INFO)
        
        Exit Sub
        End If
        
        UserList(UserIndex).Stats.GLD = UserList(UserIndex).Stats.GLD - 100000
        Dim elijepj As String
        elijepj = NameIndex(rdata)
        Dim elijeAtributo As String
        Dim elijeAtributoName As String
        Dim elijeSkill1 As String
        Dim elijeskill2 As String
        Dim elijeatrib1 As String
        Dim elijeatrib2 As String
        elijeatrib1 = RandomNumber(1, 5)
        elijeSkill1 = RandomNumber(1, 22)
        elijeskill2 = RandomNumber(1, 22)
        elijeatrib2 = RandomNumber(1, 5)
        Select Case elijeatrib2
            Case 1
             elijeAtributoName = "Inteligencia"
            Case 2
             elijeAtributoName = "Fuerza"
            Case 3
             elijeAtributoName = "Carisma"
            Case 4
             elijeAtributoName = "Constitucion"
            Case 5
             elijeAtributoName = "Agilidad"
        End Select
        Select Case elijeatrib1
            Case 1
             elijeAtributoName = "Inteligencia"
            Case 2
             elijeAtributoName = "Fuerza"
        ...
        If UserList(UserIndex).flags.Muerto = 1 Then
         ...
         'Se asegura que el target es un npc
        If UserList(UserIndex).flags.TargetNPC = 0 Then
              Call SendData(ToIndex, UserIndex, 0, "||Primero tienes que seleccionar un personaje, haz click izquierdo sobre él." & FONTTYPE_INFO)
              Exit Sub
        End If
        If Distancia(Npclist(UserList(UserIndex).flags.TargetNPC).Pos, UserList(UserIndex).Pos) > 3 Then
                  Call SendData(ToIndex, UserIndex, 0, "||Estás demasiado lejos del feriante." & FONTTYPE_INFO)
                  Exit Sub
        End If
    ...
    If Npclist(UserList(UserIndex).flags.TargetNPC).NPCtype <> 12 Then Exit Sub
        ...
        Select Case res
        Case Is > 950
        Call SendData(ToIndex, UserIndex, 0, "||" & vbWhite & "°" & " Las malas lenguas dicen que " & UserList(elijepj).Name & " tiene " & UserList(elijepj).Stats.UserAtributos(1) & " de fuerza, " & UserList(elijepj).Stats.UserAtributos(2) & " de agilidad, " & UserList(elijepj).Stats.UserAtributos(3) & " de inteligencia y " & UserList(elijepj).Stats.UserAtributos(5) & " de constitución...vaya birria, no? :PP" & "°" & str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.charindex))
        Exit Sub
        Case 861 To 950
        Call SendData(ToIndex, UserIndex, 0, "||" & vbWhite & "°" & " Me han contado que " & UserList(elijepj).Name & " sólo ha matado " & UserList(elijepj).Stats.NPCsMuertos & " monstruos, porque se lo comen vivo al tener la poquita vida de " & UserList(elijepj).Stats.MaxHP & " no me extraña...pobrecito..." & "°" & str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.charindex))
        Exit Sub
        Case 781 To 860
        Call SendData(ToIndex, UserIndex, 0, "||" & vbWhite & "°" & " ¿Pero tu no sabías que " & UserList(elijepj).Name & " es " & UserList(elijepj).Clase & "?..., pero si lo sabe hasta el mas new de XFoRCe..." & "°" & str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.charindex))
        Exit Sub
        Case 691 To 780
        Call SendData(ToIndex, UserIndex, 0, "||" & vbWhite & "°" & " ...como te iba diciendo, han visto a " & UserList(elijepj).Name & " por el mapa " & UserList(elijepj).Pos.Map & "... y digo yo que qué hará por ahí... seguro que nada bueno" & "°" & str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.charindex))
        Exit Sub
        Case 601 To 690
        If UserList(elijepj).Stats.GLD < 100000 Then
        Call SendData(ToIndex, UserIndex, 0, "||" & vbWhite & "°" & "Pobre " & UserList(elijepj).Name & ", como le asalten le robarán las " & UserList(elijepj).Stats.GLD & " monedas que con tanto sudor ganó..." & "°" & str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.charindex))
        Else
        Call SendData(ToIndex, UserIndex, 0, "||" & vbWhite & "°" & UserList(elijepj).Name & " se que lleva " & UserList(elijepj).Stats.GLD & " monedas encima... esa cantidad sólo se consigue haciendo maldades...¡si lo sabré yo que le conozco bien!" & "°" & str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.charindex))
        End If
        Exit Sub
        Case 511 To 600
        Call SendData(ToIndex, UserIndex, 0, "||" & vbWhite & "°" & " Sé de buena tinta que " & UserList(elijepj).Name & " con su level " & UserList(elijepj).Stats.ELV & " solo tiene " & UserList(elijepj).Stats.UserSkills(2) & " de magia y " & UserList(elijepj).Stats.MaxMAN & " de maná... con eso tardará dias en matar un lobo" & "°" & str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.charindex))
        Exit Sub
        Case 371 To 510
        Call SendData(ToIndex, UserIndex, 0, "||" & vbWhite & "°" & " Me he enterado de que " & UserList(elijepj).Name & " tiene " & UserList(elijepj).Stats.UserSkills(elijeSkill1) & " de " & SkillsNames(elijeSkill1) & ", " & UserList(elijepj).Stats.UserSkills(elijeskill2) & " de " & SkillsNames(elijeskill2) & " y pega por " & UserList(elijepj).Stats.MaxHIT & " de cuando en cuando..." & "°" & str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.charindex))
        Exit Sub
        Case 231 To 370
        Call SendData(ToIndex, UserIndex, 0, "||" & vbWhite & "°" & " Sí... el " & UserList(elijepj).Raza & " al que llaman " & UserList(elijepj).Name & ", dicen que su madre es una araña y su padre un zombie, y por eso tiene " & UserList(elijepj).Stats.UserAtributos(elijeatrib1) & " de " & elijeAtributoName & ", y " & UserList(elijepj).Stats.UserAtributos(elijeatrib1) & " de " & elijeAtributoName & "°" & str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.charindex))
        Exit Sub
        Case 141 To 230
        Call SendData(ToIndex, UserIndex, 0, "||" & vbWhite & "°" & " Mi vecina me ha dicho que " & UserList(elijepj).Name & " tiene " & UserList(elijepj).BancoInvent.NroItems & " cosas en el banco...es bastante new, porque con su LVL " & UserList(elijepj).Stats.ELV & " yo tenía muchas mas cosas...seguro que son todas pieles de lobo..." & "°" & str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.charindex))
        Exit Sub
        Case 51 To 140
        Call SendData(ToIndex, UserIndex, 0, "||" & vbWhite & "°" & UserList(elijepj).Name & " ha matado " & UserList(elijepj).Stats.CriminalesMatados & " criminales y " & UserList(elijepj).Stats.UsuariosMatados & " ciudadanos... habrá que ponerle una estatua por eso?" & "°" & str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.charindex))
        Exit Sub
        Case Else
        Call SendData(ToIndex, UserIndex, 0, "||" & vbWhite & "°" & " A mi me llaman chismoso, pero que sepan todos que tú eres cien veces más cotilla que yo..." & "°" & str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.charindex))
        Call SendData(ToAll, 0, 0, "||Servidor> a " & UserList(UserIndex).Name & " le encantan los chismes y es cotilla de nacimiento!!!!!" & FONTTYPE_GUILD)
        Exit Sub
        End Select
    ...
    If UCase$(Left$(rdata, 6)) = "/CMSG " Then
        ...
        If rdata <> "" And UserList(UserIndex).GuildInfo.GuildName <> "" Then
            ...
            If Cliente0113 = True Then
                Call SendData(ToGuildMembers, UserIndex, 0, "|+" & ChrW$(32) & UserList(UserIndex).Name & "> " & rdata & FONTTYPE_GUILDMSG)
            Else
                Call SendData(ToGuildMembers, UserIndex, 0, "||" & UserList(UserIndex).Name & "> " & rdata & FONTTYPE_GUILDMSG)
            End If
...
' GS
    If UCase$(Left$(rdata, 11)) = "/CENTINELA " Then
        ...
        Dim Meborra As Integer
        If rdata < 200 Then Exit Sub
        If UserList(UserIndex).flags.Codigo = rdata Then
            Call SendData(ToIndex, UserIndex, 0, "||Si sigues trabajando en 15 Minutos vuelvo." & FONTTYPE_INFO)
            UserList(UserIndex).flags.Centinela = 0
            Meborra = UserList(UserIndex).flags.TargetNPC
            Call QuitarNPC(Meborra)
            Exit Sub
        Else
            Meborra = UserList(UserIndex).flags.TargetNPC
            Call QuitarNPC(Meborra)
            Call SendData(ToIndex, UserIndex, 0, "||Codigo de verificacion incorrecto, has sido desconectado." & FONTTYPE_INFO)
...
' /GS
    ...
    Select Case UCase$(Left$(rdata, 8))
        ...
        Case "/PASSCF "
            ...
            If UCase$(rdata) = ReadField(2, UserList(UserIndex).PasswordCh, Asc("~")) Then
                ' codigo valido
