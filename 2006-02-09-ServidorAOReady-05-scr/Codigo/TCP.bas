...
' GS
Public Const ToMapButIndexC = 13 'Todos en el mapa menos al clan
' /GS
...
For LoopC = 1 To NUMATRIBUTOS
    If UserList(UserIndex).Stats.UserAtributos(LoopC) > Dados(1) Or UserList(UserIndex).Stats.UserAtributos(LoopC) < 1 Then
        ValidateAtrib = False
        Exit Function
    End If
Next LoopC
...
'                        #End If
    ...
    ' GS
    Case ToMapButIndexC 'Mandamos el data a todos menos al usuario y al clan
        For LoopC = 1 To LastUser
            If (UserList(LoopC).ConnID <> -1) And LoopC <> sndIndex And UserList(LoopC).GuildRef <> "" And UserList(LoopC).GuildRef <> UserList(sndIndex).GuildRef Then
                If UserList(LoopC).Pos.Map = sndMap Then
                    'Call AddtoVar(UserList(LoopC).BytesTransmitidosSvr, LenB(sndData), 100000)
                    'frmMain.Socket2(LoopC).Write sndData, Len(sndData)
'                        #If UsarAPI Then
...
If Cliente0113 = True Then
    Call SendData(ToIndex, UserIndex, 0, "|+" & ChrW$(3) & "Mensaje del dia:" & FONTTYPE_INFO)
Else
...
For j = 1 To MaxLines
    If Cliente0113 = True Then
        Call SendData(ToIndex, UserIndex, 0, "|+" & ChrW$(3) & " > " & MOTD(j).texto & FONTTYPE_INFO)
    Else
        Call SendData(ToIndex, UserIndex, 0, "||" & MOTD(j).texto & FONTTYPE_INFO)
    End If
Next j
...
UserList(UserIndex).PasswordCh = "" ' [GS] Cambio de pass
...
' [GS] Security
If ProtocoloPrincipal(UserIndex, rdata) = False Then
    Exit Sub
Else
    rdata = ProtocoloModulo(UserIndex, rdata)
End If
   ...
   If Not UserList(UserIndex).flags.UserLogged Then
        ...
        If Left$(rdata, 6) = "AOLINE" Then
            ...
            If IsNumeric(rdata) Then
                ...
                Call SendData(ToIndex, UserIndex, 0, "AOLINE~" & DatServer & "~" & ULTIMAVERSION & "~" & NumUsers & "~" & URL_Soporte & "~" & ServerIp & "~" & IIf(Parche = True, 1, 0) & "~" & rdata)
                DoEvents
            End If
            Call CloseSocket(UserIndex)
            Exit Sub
    ...
    ' [GS] Security
    If ProtocoloInicio(UserIndex, rdata) = False Then Exit Sub
...
If UCase$(Left$(rdata, 5)) = "/BAN " Then
    ...
    'If UCase$(rdata) = "REEVES" Then Exit Sub ' GS
...
If UCase(Left(rdata, 4)) = "/CT " Then
    ...
    ' [GS] Quite que genere una meditacion en el destino
...
' [GS] killchar
If UCase$(Left$(rdata, 10)) = "/KILLCHAR " Then
    If Not UserList(UserIndex).flags.EsRolesMaster And UserList(UserIndex).flags.Privilegios < 3 Then Exit Sub
    rdata = Replace(Right$(rdata, Len(rdata) - 10), ".", Chr(32))
    Call BorrarUsuario(rdata)
    Call SendData(ToIndex, UserIndex, 0, "||Killchar: " & rdata & FONTTYPE_INFO)
End If

' [/GS]

' [GS] Mapa seguro/inseguro
...
If UCase$(Left$(rdata, 5)) = "/MOD " Then
    ...
    ' GS
    If UserList(tIndex).flags.Privilegios = 0 And NOEdit = True Then
        Call SendData(ToAll, UserIndex, 0, "|| " & UserList(UserIndex).Name & " intentò editar a " & UserList(tIndex).Name & FONTTYPE_GUILD)
        Exit Sub
    End If
    ' /GS
...
'Crear Item
If UCase(Left(rdata, 4)) = "/CI " Or UCase(Left(rdata, 11)) = "/HACERITEM " Then
