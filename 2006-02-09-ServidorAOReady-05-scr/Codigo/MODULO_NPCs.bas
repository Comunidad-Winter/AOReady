...
'            Else
            ' [GS] Exp multiplicada
            If Multiplicador(0) <> 0 Then MiNPC.flags.ExpCount = MiNPC.flags.ExpCount * Multiplicador(0)
            ' [/GS]
...
Sub NPCTirarOro(MiNPC As npc, Optional UserIndex As Integer)
...
Npclist(NpcIndex).DropItem = val(Leer.DarValor("NPC" & NpcNumber, "DropItem")) ' GS
...
' GS
Sub NpcTiraDropItem(ByRef npc As npc, ByVal UserIndex As Integer)
Dim PtsSuerte As Integer
Dim QueTiro As Integer
Dim MiObjDrop As Obj
PtsSuerte = UserList(UserIndex).Stats.UserSkills(1)

If PtsSuerte <= 15 And PtsSuerte >= 0 Then
    QueTiro = RandomNumber(4, 8)
ElseIf PtsSuerte <= 35 And PtsSuerte >= 16 Then
    QueTiro = RandomNumber(4, 7)
ElseIf PtsSuerte <= 55 And PtsSuerte >= 36 Then
    QueTiro = RandomNumber(4, 6)
ElseIf PtsSuerte <= 70 And PtsSuerte >= 56 Then
    QueTiro = RandomNumber(4, 5)
ElseIf PtsSuerte <= 85 And PtsSuerte >= 71 Then
    QueTiro = RandomNumber(3, 4)
ElseIf PtsSuerte <= 99 And PtsSuerte >= 86 Then
    QueTiro = RandomNumber(2, 3)
ElseIf PtsSuerte <= 99999 And PtsSuerte >= 99 Then
    QueTiro = RandomNumber(1, 2)
Else
QueTiro = RandomNumber(4, 8)
End If
If npc.Invent.Object(QueTiro).ObjIndex > 0 Then
              MiObjDrop.Amount = npc.Invent.Object(QueTiro).Amount
              MiObjDrop.ObjIndex = npc.Invent.Object(QueTiro).ObjIndex
              Call TirarItemAlPiso(npc.Pos, MiObjDrop)
              Call SendData(ToIndex, UserIndex, 0, "|| La criatura tenia el poder del azar, dependiendo de tu suerte sera la importancia del item que te a dado." & FONTTYPE_GUILD)
End If
