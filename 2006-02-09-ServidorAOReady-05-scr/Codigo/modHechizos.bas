...
' [GS] ExclusivoClase
If LenB(Hechizos(HechizoIndex).ExclusivoClase) > 0 And Hechizos(HechizoIndex).ExclusivoClase <> UserList(UserIndex).Clase Then
    Call SendData(ToIndex, UserIndex, 0, "||Tú clase no puede lanzar este hechizo." & FONTTYPE_INFO)
    PuedeLanzar = False
    Exit Function
End If
' [/GS]
...
If Hechizos(H).Invisibilidad = 1 Then
   ...
   ' GS
   If CfgInvi = 1 Then
        Call SendData(ToMapButIndexC, TU, UserList(TU).Pos.Map, "NOVER" & UserList(TU).Char.charindex & ",1")
        Call SendData(ToIndex, TU, UserList(TU).Pos.Map, "SIVER" & UserList(TU).Char.charindex & ",1")
    Else
...
ElseIf Hechizos(H).SubeHP = 2 Then
    ...
    If UserList(UserIndex).flags.SeguroClan Then
        If UserList(tempChr).GuildInfo.GuildName = UserList(UserIndex).GuildInfo.GuildName And UserList(UserIndex).GuildInfo.GuildName <> "" Then
...
Sub AreaHechizo(UserIndex As Integer, NpcIndex As Integer)
    ...
    Call CalcularDarExp(UserIndex, NpcIndex, Hit)
