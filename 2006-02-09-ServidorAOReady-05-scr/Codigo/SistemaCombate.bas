    ...
    If NpcIndex > 0 Then
        ...
        Else ' daño comun
           If Arma.proyectil = 1 Then
                ...
                ' GS
                If Arma.Doble = 1 And Doble = 2 Then
                    DañoArma = RandomNumber(Arma.MinHIT, Arma.MaxHIT) * 2
                    DañoMaxArma = Arma.MaxHIT * 2
                Else ' /GS
                    DañoArma = RandomNumber(Arma.MinHIT, Arma.MaxHIT)
                    DañoMaxArma = Arma.MaxHIT
                End If
...
If UserList(AttackerIndex).flags.SeguroClan Then
    If UserList(VictimIndex).GuildInfo.GuildName = UserList(AttackerIndex).GuildInfo.GuildName And UserList(AttackerIndex).GuildInfo.GuildName <> "" Then
        Call SendData(ToIndex, AttackerIndex, 0, "||No puedes atacar a tu propio Clan con el seguro activado, escribe /SEGCLAN para desactivarlo." & FONTTYPE_FIGHT)
        Exit Function
    End If
End If
' [/GS]
...
If ExpaDar > 0 Then
    ' [GS] Multiplicar Exp
    If Multiplicador(0) <> 0 Then ExpaDar = ExpaDar * Multiplicador(0)
    ' [/GS]
